...
class StorageContract:
    ...
    def __init__(self, web3: Web3, contract_address: HexAddress):
        ...
        self.abi = [
            ...
            {
                "inputs": [{"internalType": "bytes", "name": "fileCID", "type": "bytes"}],
            ...
            {
                "inputs": [
                    {"internalType": "uint256", "name": "cidsLength", "type": "uint256"},
                    {"internalType": "uint256", "name": "sizesLength", "type": "uint256"},
                ],
            ...
            },
            {
                "anonymous": False,
                "inputs": [
                    {"indexed": True, "internalType": "bytes32", "name": "blockId", "type": "bytes32"},
                    {"indexed": True, "internalType": "bytes", "name": "peerId", "type": "bytes"},
                ],
            ...
            },
            {
                "inputs": [],
                "name": "accessManager",
                "outputs": [{"internalType": "contract IAccessManager", "name": "", "type": "address"}],
                "stateMutability": "view",
                "type": "function",
            },
            {
            ...
            },
            {
                "inputs": [
                    ...
                    {"internalType": "bool", "name": "isReplica", "type": "bool"},
                ],
                "name": "addPeerBlock",
                ...
                "outputs": [
                    {
                        "components": [
                            ...
                            {
                                "components": [
                                    {"internalType": "bytes[]", "name": "chunkCIDs", "type": "bytes[]"},
            ...
            {
                "inputs": [
                    {"internalType": "bytes[]", "name": "cids", "type": "bytes[]"},
                    {"internalType": "bytes32", "name": "bucketId", "type": "bytes32"},
            ...
            },
            {
                "inputs": [],
                "name": "token",
                "outputs": [{"internalType": "contractIAkaveToken", "name": "", "type": "address"}],
                "stateMutability": "view",
                "type": "function",
            },
            {
            ...
            },
            {
                "inputs": [
                    {
                        "components": [
                            {"internalType": "bytes32", "name": "blockCID", "type": "bytes32"},
            ...
            {
                "inputs": [{"internalType": "address", "name": "accessManagerAddress", "type": "address"}],
        ...
        try:
            ...
            if "Could not format invalid value" in error_msg and "as field 'abi'" in error_msg:
                ...
                raise ValueError(
                    f"Failed to initialize storage contract at {contract_address}: {type(e).__name__}"
                ) from e
    ...
    def create_bucket(
        self, bucket_name: str, from_address: HexAddress, private_key: str, gas_limit: int = None, nonce_manager=None
    ) -> HexStr:
        ...
        if receipt.status != 1:
            ...
            try:
                self.contract.functions.createBucket(bucket_name).call({"from": from_address})
            except Exception as e:
    ...
    def create_file(
        self, from_address: HexAddress, private_key: str, bucket_id: bytes, file_name: str, nonce_manager=None
    ) -> HexStr:
        ...
        if receipt.status != 1:
            # Get revert reason if possible
            try:
                self.contract.functions.createFile(bucket_id, file_name).call({"from": from_address})
            except Exception as e:
                raise Exception(f"Transaction reverted: {str(e)}")
            raise Exception(f"Transaction failed. Receipt: {receipt}")
    ...
    def commit_file(
        ...
        tx = self.contract.functions.commitFile(
            bucket_id, file_name, encoded_size, actual_size, root_cid
        ).build_transaction(
            {
                "from": from_address,
    ...
    def delete_bucket(
        self, bucket_name: str, from_address: HexAddress, private_key: str, bucket_id_hex: str = None
    ) -> HexStr:
    ...
    def get_bucket_by_name(
        self, call_opts: dict, bucket_name: str, owner_address: str = None, file_offset: int = 0, file_limit: int = 10
    ):
    ...
    def add_file_chunks(
        ...
        chunk_blocks_cids: List[List[bytes]],
        chunk_block_sizes: List[List[int]],
    ...
    def get_owner_buckets(
        self, owner_address: HexAddress, offset: int = 0, limit: int = 10, file_offset: int = 0, file_limit: int = 10
    ):
        return self.contract.functions.getOwnerBuckets(owner_address, offset, limit, file_offset, file_limit).call()

    def initialize_contract(
        self, from_address: HexAddress, private_key: str, token_address: HexAddress, nonce_manager=None
    ) -> HexStr:
        tx_params = {
            "from": from_address,
            "gas": 500000,
    ...
    def delete_peer_block(
        ...
        tx = self.contract.functions.deletePeerBlock(block_id, peer_id, cid, file_name, index).build_transaction(
            tx_params
        )
        signed_tx = Account.sign_transaction(tx, private_key)
    ...
    def fill_chunk_block(
        self, from_address: HexAddress, private_key: str, fill_args: dict, nonce_manager=None
    ) -> HexStr:
        tx_params = {
            "from": from_address,
            "gas": 1000000,
    ...
    def fill_chunk_blocks(
        self, from_address: HexAddress, private_key: str, fill_args_list: List[dict], nonce_manager=None
    ) -> HexStr:
        tx_params = {
            "from": from_address,
            "gas": 2000000,
    ...
    def get_buckets_by_ids_with_files(
        self,
        bucket_ids: List[bytes],
        bucket_offset: int = 0,
        bucket_limit: int = 10,
        file_offset: int = 0,
        file_limit: int = 10,
    ...
    def set_access_manager(
        self, from_address: HexAddress, private_key: str, access_manager_address: HexAddress, nonce_manager=None
    ) -> HexStr:
        tx_params = {
            "from": from_address,
            "gas": 500000,
