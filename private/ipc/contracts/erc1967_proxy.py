...
class ERC1967ProxyTransactor:
    ...
    def fallback(self, account: LocalAccount, calldata: bytes, tx_params: Optional[Dict[str, Any]] = None) -> HexStr:
        ...
        Returns:
            Transaction hash
        """
        params = {"from": account.address, "gasPrice": self.web3.eth.gas_price, "data": calldata}
        if tx_params:
...
class ERC1967ProxyFilterer:
    ...
    def filter_upgraded(
        self, from_block: int = 0, to_block: str = "latest", implementation: Optional[List[Address]] = None
    ):
        """
...
class ERC1967Proxy:
    ...
    def __init__(self, web3: Web3, contract_address: HexAddress):
        ...
        self.contract = web3.eth.contract(address=contract_address, abi=ERC1967ProxyMetaData.ABI)
    ...
    def deploy(
        ...
        """
        contract_factory = web3.eth.contract(abi=ERC1967ProxyMetaData.ABI, bytecode=ERC1967ProxyMetaData.BIN)
    ...
    def call_implementation(self, method_sig: str, args: List[Any] = None, caller: Optional[Address] = None) -> Any:
        """
        ...
        encoded_args = self.web3.codec.encode(["address"] + [type(arg).__name__ for arg in args], args) if args else b""
    ...
    def transact_implementation(
        self, account: LocalAccount, method_sig: str, args: List[Any] = None, tx_params: Optional[Dict[str, Any]] = None
    ) -> HexStr:
        """
...
def deploy_erc1967_proxy(
    web3: Web3, account: LocalAccount, implementation: Address, data: bytes, tx_params: Optional[Dict[str, Any]] = None
) -> ERC1967Proxy:
    """
    Deploy a new ERC1967Proxy contract.
