...
class PDPVerifierMetaData:
    ABI = [
        ...
        {
            "anonymous": False,
            "inputs": [{"indexed": True, "internalType": "uint256", "name": "setId", "type": "uint256"}],
            "name": "ProofSetEmpty",
            "type": "event",
        },
        {
...
class PDPVerifier:
    def __init__(self, w3: Web3, address: str):
        ...
        self.contract = w3.eth.contract(address=self.address, abi=PDPVerifierMetaData.ABI)
    ...
    def add_roots(
        self, account: LocalAccount, set_id: int, root_data: List[Tuple], extra_data: bytes, gas_limit: int = 1000000
    ) -> str:
        tx = self.contract.functions.addRoots(set_id, root_data, extra_data).build_transaction(
            {
    ...
    def create_proof_set(
        self, account: LocalAccount, listener_addr: str, extra_data: bytes, value: int = 0, gas_limit: int = 1000000
    ) -> str:
        tx = self.contract.functions.createProofSet(listener_addr, extra_data).build_transaction(
            {
                "from": account.address,
                "value": value,
                "gas": gas_limit,
    ...
    def next_proving_period(
        self, account: LocalAccount, set_id: int, challenge_epoch: int, extra_data: bytes, gas_limit: int = 1000000
    ) -> str:
        tx = self.contract.functions.nextProvingPeriod(set_id, challenge_epoch, extra_data).build_transaction(
            {
                "from": account.address,
                "gas": gas_limit,
                "nonce": self.w3.eth.get_transaction_count(account.address),
    ...
    def propose_proof_set_owner(
        self, account: LocalAccount, set_id: int, new_owner: str, gas_limit: int = 1000000
    ) -> str:
    ...
    def prove_possession(
        self, account: LocalAccount, set_id: int, proofs: List[Tuple], value: int = 0, gas_limit: int = 1000000
    ) -> str:
    ...
    def schedule_removals(
        self, account: LocalAccount, set_id: int, root_ids: List[int], extra_data: bytes, gas_limit: int = 1000000
    ) -> str:
        tx = self.contract.functions.scheduleRemovals(set_id, root_ids, extra_data).build_transaction(
            {
                "from": account.address,
                "gas": gas_limit,
                "nonce": self.w3.eth.get_transaction_count(account.address),
...
def deploy_pdp_verifier(
    w3: Web3, account: LocalAccount, challenge_finality: int, gas_limit: int = 3000000
) -> Tuple[str, str]:

    contract_factory = w3.eth.contract(abi=PDPVerifierMetaData.ABI, bytecode=PDPVerifierMetaData.BIN)
