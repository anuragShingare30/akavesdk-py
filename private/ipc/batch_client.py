...
class BatchClient:
    ...
    def get_transaction_receipts_batch(
        self, requests: List[BatchReceiptRequest], timeout: float = 30.0
    ) -> BatchReceiptResult:
        ...
        try:
            ...
            for i, (req, raw_response) in enumerate(zip(requests, raw_responses)):
                if "error" in raw_response:
                    ...
                    response = BatchReceiptResponse(receipt=None, error=Exception(error_msg), key=req.key)
                elif raw_response.get("result") is None:
                    response = BatchReceiptResponse(
                        receipt=None, error=TransactionNotFound(f"Transaction {req.hash} not found"), key=req.key
                    )
                else:
                    response = BatchReceiptResponse(receipt=raw_response["result"], error=None, key=req.key)
    ...
    def get_blocks_batch(self, block_numbers: List[int]) -> List[BatchBlockResponse]:
        batch_requests = []
        ...
        try:
            ...
            for block_number, raw_response in zip(block_numbers, raw_responses):
                if "error" in raw_response:
                    error_msg = raw_response["error"].get("message", "Unknown error")
                    response = BatchBlockResponse(block_number=block_number, block=None, error=Exception(error_msg))
                elif raw_response.get("result") is None:
                    # Block not found
                    response = BatchBlockResponse(
                        block_number=block_number, block=None, error=BlockNotFound(f"Block {block_number} not found")
                    )
                else:
                    try:
                        block_json = json.dumps(raw_response["result"]).encode("utf-8")
                        block_data = block_from_json(block_json)
                        response = BatchBlockResponse(block_number=block_number, block=block_data, error=None)
                    except Exception as e:
                        response = BatchBlockResponse(block_number=block_number, block=None, error=e)
                responses.append(response)
            ...
            for block_number in block_numbers:
                try:
                    ...
                    response = BatchBlockResponse(block_number=block_number, block=None, error=err)
                responses.append(response)
