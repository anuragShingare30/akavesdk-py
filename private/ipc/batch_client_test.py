...
def test_get_transaction_receipts_batch():
    ...
    # Deploy contracts
    client = Client.deploy_contracts(Config(dial_uri=dial_uri, private_key=pk_hex))
    ...
    for i in range(num_txs):
        ...
        requests.append(BatchReceiptRequest(hash=tx_hash.hex(), key=f"tx-{i}"))
    ...
    for i, response in enumerate(result.responses):
        ...
        if response.error is not None:
            ...
            print(f"Transaction {tx_hash} mined in block {block_number} " f"with status {response.receipt['status']}")
