...
def new_funded_account(
    source_private_key: str, dial_uri: str, amount_wei: int, max_retries: int = 10, retry_delay: float = 0.01
) -> LocalAccount:
    try:
...
def deposit(web3: Web3, dest_address: str, source_account: LocalAccount, amount_wei: int, chain_id: int) -> None:
...
def wait_for_tx(web3: Web3, tx_hash: Union[str, bytes], timeout: float = 120.0, poll_interval: float = 0.2):
