...
class MockStorageContract:
    ...
    def create_file(self, from_address: str, private_key: str, bucket_id: bytes, file_name: str, **kwargs) -> str:
        file_id = f"file_{len(self._files)}"
        self._files[file_name] = {"id": file_id, "name": file_name, "bucket_id": bucket_id, "created_at": 1234567890}
        return f"0x{file_id}"
    ...
    def commit_file(
        self, bucket_name: str, file_name: str, size: int, root_cid: bytes, from_address: str, private_key: str
    ) -> str:
        if file_name in self._files:
